...
The [Flag Evaluation API](/api-docs/flag-evaluation-api) can also evaluate flags by tag. Use the `filter.tags` field to request feature flag evaluation results for one or more tags, and use `filter.tagFilterMode` to decide whether multiple tags should be matched with `and` or `or`.

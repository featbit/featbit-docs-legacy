...
A feature flag lifecycle is more than creating a flag and deleting it later. It should connect the flag's purpose, expected lifetime, release evidence, code references, audit history, and cleanup plan. Coding agents can help teams keep that lifecycle visible inside the codebase and connect it with FeatBit data.
...
### 3. Create flags with agent guidance

Use project-level agent skills or repository instructions to guide how new flags should be named, owned, documented, and reviewed.

Read more: [Create flags with agent guidance](flag-lifecycle-management/create-flags-with-agent-guidance)
...
### 5. Control exposure and release decisions

Coordinate flag targeting, rollout percentage, protected audiences, and decision categories such as continue, pause, rollback, or iterate.

Read more: [Control exposure and release decisions](flag-lifecycle-management/control-exposure-and-release-decisions)
...
### 7. Capture release learning

Summarize what changed, who was exposed, what happened, and what the next iteration should be. The result should feed the next release decision instead of being lost in chat or pull request history.

Read more: [Capture release learning](flag-lifecycle-management/capture-release-learning)

### 8. Detect stale feature flags
...
### 9. Clean up feature flags with coding agents
...
### 10. Manage flag references with coding agents

Track where flags are used across the codebase, understand fallback values, and keep code references aligned with FeatBit state.

Read more: [Manage flag references with coding agents](flag-lifecycle-management/manage-flag-references-with-coding-agents)

### 11. Review flag audit history

Use FeatBit audit history during code review to understand whether a code change affects a historical release decision, an active experiment, or a cleanup task.

Read more: [Review flag audit history](flag-lifecycle-management/review-flag-audit-history)
...
- FeatBit CLI, so the coding agent can retrieve feature flag details, audit history, rollout state, and experiment context directly from FeatBit.

With these pieces in place, the coding agent can work from both sides of the lifecycle: it can inspect code references in the repository and inspect flag state in FeatBit.
